...
mapJS.loadMarkersFromKMLAsync = function(kmlURL, map) {
    ...
    $.ajax({
        url: 'http://xml2json.herokuapp.com/?callback=json&url=' + kmlURL,  // translate xml to json using online service.
        type: 'GET',
        crossDomain: true,
        dataType: 'jsonp'
    }).done(function(data) {
...
mapJS.attachSingletonInfoWindow = function(markers) {
    var prevInfoWindow = null;
    for (var i in markers) {
        var marker = markers[i];

        var infowindow = mapJS.createInfoWindow(marker.getTitle(), marker.description);
        marker.infoWindow = infowindow;
        google.maps.event.addListener(marker, 'mouseover', function(marker, infowindow) {
            return function() {
                if (mapJS.isInfoWindowOpen(infowindow))
                    return;
